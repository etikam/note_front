...
import { fetchDetailedReport } from '@/features/teacherFacultyDashboard/report/api/detailedReportApi'
import { createOverviewFilters } from '@/features/teacherFacultyDashboard/overview/model/overviewFilterDefaults'

/**
 * Données et filtres pour la page Rapports (tableau détaillé par niveau).
 *
 * @param {string | null} academicYearId
 * @param {boolean} enabled
 * @param {{ managedDeptId?: number | null, institutionWide?: boolean }} scope
 */
export function useDetailedReport(academicYearId, enabled, scope = {}) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const body = await fetchDetailedReport(apiParams)
      setData(body)
    } catch (e) {
      setError(e?.message ?? 'Impossible de charger le rapport détaillé.')
      setData(null)
    } finally {
      setLoading(false)
    }
