...
/**
 * @param {{ canViewReports?: boolean }} props
 */
export function ImportPageHeader({ canViewReports = false }) {
  return (
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p className="mt-1.5 text-sm text-zinc-500 dark:text-zinc-400 max-w-2xl">
          Importez des dossiers étudiants à partir d’un fichier CSV institutionnel. Taille max. {MAX_FILE_SIZE_LABEL} — l’aperçu et le contrôle des en-têtes se font sur votre appareil avant envoi au serveur.
        </p>
      </div>
      {canViewReports ? (
        <Button
